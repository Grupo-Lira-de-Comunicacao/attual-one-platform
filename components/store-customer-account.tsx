...
export function StoreCustomerAccount({ slug }: { slug: string }) {
  ...
  async function claimLocalOrders() {
    ...
    const claims = local
      .filter((order) => !cloudIds.has(order.id) && Boolean(order.claimToken))
      .map((order) => ({ orderId:order.id, claimToken:String(order.claimToken) }))
      .slice(0, 20);
    if (!claims.length) {
      setMessage("Nenhum pedido deste aparelho possui o comprovante seguro necessário para vínculo automático.");
      return;
    }
    ...
    try {
      ...
      const body = await response.json() as { linked?:number; rejected?:number; error?:string };
      ...
      await refreshCloud();
      setMessage(linked > 0
        ? `${linked} pedido(s) comprovado(s) e vinculado(s) à sua conta.${rejected > 0 ? ` ${rejected} não puderam ser confirmados.` : " Agora eles aparecem nos outros dispositivos após você entrar com o mesmo e-mail."}`
        : "Nenhum pedido pôde ser comprovado para vínculo automático.");
    } catch (error) {
  ...
  const { localOrdersPendingClaim, legacyLocalOrdersPendingClaim } = useMemo(() => {
    if (!cloud) return { localOrdersPendingClaim:0, legacyLocalOrdersPendingClaim:0 };
    const cloudIds = new Set((cloud.orders ?? []).map((order) => order.id));
    const pending = orders.filter((order) => !cloudIds.has(order.id));
    return {
      localOrdersPendingClaim: pending.filter((order) => Boolean(order.claimToken)).length,
      legacyLocalOrdersPendingClaim: pending.filter((order) => !order.claimToken).length,
    };
  }, [cloud, orders]);
      ...
      <aside className="cart-drawer" role="dialog" aria-modal="true" aria-label="Minha conta">
        ...
        <div className="cart-items">
          ...
          <section className="border-t border-slate-100 py-5">
            ...
            {cloud&&localOrdersPendingClaim>0&&<div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-3"><strong className="text-sm text-blue-950">Levar pedidos comprovados deste aparelho para minha conta</strong><p className="mt-1 text-xs leading-5 text-blue-900">Você tem {localOrdersPendingClaim} pedido(s) com comprovante seguro salvo somente neste navegador. O vínculo exige esse comprovante criptográfico além do acesso à sua conta.</p><button className="store-primary mt-3 w-full" disabled={busy} onClick={()=>void claimLocalOrders()}>{busy?"Vinculando...":"Vincular pedidos comprovados"}</button></div>}{cloud&&legacyLocalOrdersPendingClaim>0&&<div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-950"><strong>{legacyLocalOrdersPendingClaim} pedido(s) antigo(s) deste aparelho não possuem comprovante seguro.</strong><p className="mt-1">Eles continuam visíveis localmente, mas não serão associados automaticamente por UUID ou telefone. A recuperação desses pedidos exige um fluxo assistido separado.</p></div>}
