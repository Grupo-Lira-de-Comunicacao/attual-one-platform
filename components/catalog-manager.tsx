...
export function CatalogManager({ initialView, companyId }: { initialView: View; companyId?: string }) {
  ...
  const filteredProducts = useMemo(() => state?.products.filter(product => {
    ...
    return text.includes(search.toLowerCase()) && (categoryFilter === "all" || product.categoryId === categoryFilter) && (statusFilter === "all" || product.status === statusFilter);
  }) ?? [], [state, search, categoryFilter, statusFilter]);

  if (!state) return <div className="page"><div className="catalog-loading">Carregando catálogo...</div></div>;
  ...
  return <div className="page catalog-page">
    ...
    <section className="page-heading catalog-heading"><div><p className="eyebrow">CATÁLOGO · HAMBURGUERIA 07</p><h1>{title}</h1><p>{view === "stock" ? "Controle saldos e registre cada movimentação." : "Organize o cardápio que seus clientes encontram."}</p></div><button className="primary-button" onClick={() => view === "categories" ? (setEditingCategory(null), setCategoryForm({ name: "", description: "", status: "active", displayOrder: state.categories.length + 1 })) : view === "stock" ? setMovementProduct(state.products[0] ?? null) : openProduct()}><Plus size={18}/>{view === "categories" ? "Nova categoria" : view === "stock" ? "Movimentar estoque" : "Novo produto"}</button></section>
    <div className="catalog-tabs"><button className={view === "products" ? "active" : ""} onClick={() => setView("products")}><Package size={17}/>Produtos <em>{state.products.length}</em></button><button className={view === "categories" ? "active" : ""} onClick={() => setView("categories")}><Boxes size={17}/>Categorias <em>{state.categories.length}</em></button><button className={view === "stock" ? "active" : ""} onClick={() => setView("stock")}><AlertTriangle size={17}/>Estoque {lowCount > 0 && <em className="warning">{lowCount}</em>}</button></div>

    {view === "products" && <>
      <div className="catalog-toolbar"><label className="catalog-search"><Search size={17}/><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por nome ou SKU..."/></label><Select value={categoryFilter} onChange={setCategoryFilter} options={[['all','Todas as categorias'], ...state.categories.map(c => [c.id,c.name])]}/><Select value={statusFilter} onChange={setStatusFilter} options={[["all","Todos os status"],["available","Disponível"],["out_of_stock","Esgotado"],["inactive","Inativo"]]}/></div>
      <div className="catalog-table-wrap"><table className="catalog-table"><thead><tr><th>Produto</th><th>Categoria</th><th>Preço</th><th>Estoque</th><th>Status</th><th><span className="sr-only">Ações</span></th></tr></thead><tbody>{filteredProducts.map(product => { const category = state.categories.find(c => c.id === product.categoryId); const low = product.trackStock && product.currentStock <= product.minimumStock; return <tr key={product.id}><td><div className="product-cell"><span className="catalog-product-image">{product.imageUrl ? <ProductImage src={product.imageUrl} /> : <Package/>}</span><span><strong>{product.name}</strong><small>{product.sku || "Sem SKU"}</small></span></div></td><td>{category?.name}</td><td><strong>{product.promotionalPrice !== undefined ? money.format(product.promotionalPrice) : money.format(product.price)}</strong>{product.promotionalPrice !== undefined && <small className="old-price">{money.format(product.price)}</small>}</td><td><span className={low ? "stock-low" : ""}>{product.trackStock ? `${product.currentStock} un.` : "Sem controle"}</span>{low && <small className="low-label">Estoque baixo</small>}</td><td><Status status={product.status}/></td><td><div className="row-actions"><button title="Editar" onClick={() => openProduct(product)}><Edit3 size={16}/></button><button title="Excluir" className="danger" onClick={() => removeProduct(product)}><Trash2 size={16}/></button></div></td></tr>})}</tbody></table>{filteredProducts.length === 0 && <div className="table-empty">Nenhum produto encontrado.</div>}</div>
    </>}

    {view === "categories" && <div className="category-grid">{[...state.categories].sort((a,b) => a.displayOrder - b.displayOrder).map(category => { const count = state.products.filter(p => p.categoryId === category.id).length; return <article className="category-card" key={category.id}><div className="category-order">{String(category.displayOrder).padStart(2,"0")}</div><div><div className="category-title"><h3>{category.name}</h3><span className={category.status}>{category.status === "active" ? "Ativa" : "Inativa"}</span></div><p>{category.description || "Sem descrição"}</p><small>{count} {count === 1 ? "produto" : "produtos"}</small></div><div className="row-actions"><button title="Editar" onClick={() => { setEditingCategory(category.id); setCategoryForm({ name: category.name, description: category.description, status: category.status, displayOrder: category.displayOrder }); }}><Edit3 size={16}/></button><button title="Excluir" className="danger" onClick={() => removeCategory(category)}><Trash2 size={16}/></button></div></article>})}</div>}
...
function Select({value,onChange,options}:{value:string;onChange:(value:string)=>void;options:string[][]}) { return <label className="catalog-select"><select value={value} onChange={e=>onChange(e.target.value)}>{options.map(([v,l])=><option value={v} key={v}>{l}</option>)}</select><ChevronDown size={15}/></label>; }
function ProductImage({ src }: { src: string }) { return <span className="remote-product-image" style={{ backgroundImage: `url(${src})` }} role="img" aria-label="Imagem do produto" />; }
