...
export function ReportsManager({companyId}:{companyId?:string}){
 ...
 useEffect(()=>{let cancelled=false;repo().analytics.getSnapshot(period).then(s=>{if(!cancelled)setData(s)}).catch(e=>{if(!cancelled)inform("error",e instanceof Error?e.message:"Não foi possível carregar os indicadores.")});return()=>{cancelled=true};
  // eslint-disable-next-line react-hooks/exhaustive-deps
 },[period,companyId]);
 useEffect(()=>{let cancelled=false;repo().analytics.getProductsReport(period,search,productsPage,PAGE_SIZE).then(r=>{if(!cancelled)setProducts(r)}).catch(e=>{if(!cancelled)inform("error",e instanceof Error?e.message:"Não foi possível carregar os produtos.")});return()=>{cancelled=true};
  // eslint-disable-next-line react-hooks/exhaustive-deps
 },[period,search,productsPage,companyId]);
 useEffect(()=>{let cancelled=false;repo().analytics.getCustomersReport(period,search,customersPage,PAGE_SIZE).then(r=>{if(!cancelled)setCustomers(r)}).catch(e=>{if(!cancelled)inform("error",e instanceof Error?e.message:"Não foi possível carregar os clientes.")});return()=>{cancelled=true};
  // eslint-disable-next-line react-hooks/exhaustive-deps
 },[period,search,customersPage,companyId]);
 useEffect(()=>{let cancelled=false;repo().analytics.getStockReport(search,stockPage,PAGE_SIZE).then(r=>{if(!cancelled)setStock(r)}).catch(e=>{if(!cancelled)inform("error",e instanceof Error?e.message:"Não foi possível carregar o estoque.")});return()=>{cancelled=true};
  // eslint-disable-next-line react-hooks/exhaustive-deps
 ...
 if(!data)return <div className="page">{toast}<div className="catalog-loading">Preparando relatórios...</div></div>;
