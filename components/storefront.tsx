...
import { ArrowLeft, Check, ChevronRight, Clock3, MapPin, Minus, Plus, Search, ShoppingBag, Store, X } from "lucide-react";
...
export function Storefront({ slug }: { slug: string }) {
  ...
  useEffect(()=>{
    const listener = (event:Event)=>{
      ...
      for(const item of detail){
        ...
        next.push({...item,productName:product.name,unitPrice:product.promotionalPrice??product.price,quantity});
      }
  ...
  const products = useMemo(()=>store?.products.filter((product)=>(category==="all"||product.categoryId===category)&&`${product.name} ${product.description}`.toLowerCase().includes(search.toLowerCase()))??[],[store,category,search]);
  ...
  function addItem(product:PublicStoreProduct){
    ...
    setCart((current)=>{
      ...
      return [...current,{productId:product.id,productName:product.name,unitPrice:product.promotionalPrice??product.price,quantity:1,additions:[],note:""}];
    });
  ...
  return <div className="public-store">
    ...
    <main id="cardapio" className="store-content"><section className="store-toolbar"><label><Search/><input value={search} onChange={(event)=>setSearch(event.target.value)} placeholder="Buscar produtos"/></label><nav aria-label="Categorias"><button className={category==="all"?"active":""} onClick={()=>setCategory("all")}>Todos</button>{store.categories.map((item)=><button key={item.id} className={category===item.id?"active":""} onClick={()=>setCategory(item.id)}>{item.name}</button>)}</nav></section>{products.length===0?<div className="store-empty"><Search/><h2>Nenhum produto encontrado</h2><p>Tente outra busca ou categoria.</p></div>:<section className="store-products">{products.map((product)=><ProductCard key={product.id} product={product} companyName={store.config.name} disabled={!store.config.open&&!store.config.acceptOrdersWhenClosed} onAdd={()=>addItem(product)}/>)}</section>}</main>
    ...
    {checkout&&<CheckoutModal slug={slug} store={store} cart={cart} subtotal={subtotal} onClose={()=>setCheckout(false)} onConfirmed={(order)=>{setCheckout(false);setCart([]);setConfirmed(order);window.dispatchEvent(new Event("attual-one:customer-memory-updated"));}}/>}
...
function CheckoutModal({slug,store,cart,subtotal,onClose,onConfirmed}:{slug:string;store:PublicStorePayload;cart:CartItem[];subtotal:number;onClose:()=>void;onConfirmed:(order:PublicCheckoutResult)=>void}){
  ...
  const [payment,setPayment]=useState<PublicPaymentMethod>("pix"),[coupon,setCoupon]=useState(""),[error,setError]=useState(""),[sending,setSending]=useState(false);
  ...
  async function submit(){
    ...
    const input:PublicCheckoutInput={submissionId,identified,name,phone,fulfillment,address:fulfillment==="delivery"?address:undefined,paymentMethod:payment,couponCode:coupon.trim()||undefined,items:cart.map((item)=>({productId:item.productId,quantity:item.quantity,additions:item.additions,note:item.note}))};
    try{
  ...
  return <div className="store-overlay checkout-overlay"><section className="checkout-modal"><header><button onClick={onClose}><ArrowLeft/></button><div><p className="eyebrow">ÚLTIMA ETAPA</p><h2>Finalizar pedido</h2></div></header><div className="checkout-body"><section><h3>Identificação</h3><label className="anonymous-check"><input type="checkbox" checked={!identified} onChange={(event)=>setIdentified(!event.target.checked)}/>Comprar sem identificação</label>{identified&&<div className="checkout-grid"><label>Nome<input value={name} onChange={(event)=>setName(event.target.value)}/></label><label>Telefone<input value={phone} onChange={(event)=>setPhone(event.target.value)}/></label></div>}</section><section><h3>Como você quer receber?</h3><div className="checkout-options">{[["pickup","Retirada"],["delivery","Entrega"],["dine_in","Consumo local"]].map(([value,label])=><button key={value} className={fulfillment===value?"active":""} onClick={()=>setFulfillment(value as PublicFulfillmentType)}>{label}</button>)}</div>{fulfillment==="delivery"&&<div className="checkout-grid address"><label>Rua<input value={address.street} onChange={(event)=>setAddress({...address,street:event.target.value})}/></label><label>Número<input value={address.number} onChange={(event)=>setAddress({...address,number:event.target.value})}/></label><label>Bairro<input value={address.district} onChange={(event)=>setAddress({...address,district:event.target.value})}/></label><label>Complemento<input value={address.complement} onChange={(event)=>setAddress({...address,complement:event.target.value})}/></label></div>}</section><section><h3>Pagamento</h3><div className="checkout-options">{[["pix","Pix"],["cash","Dinheiro"],["credit_card","Crédito"],["debit_card","Débito"]].map(([value,label])=><button key={value} className={payment===value?"active":""} onClick={()=>setPayment(value as PublicPaymentMethod)}>{label}</button>)}</div></section><section><h3>Cupom</h3><div className="coupon-input"><input value={coupon} onChange={(event)=>setCoupon(event.target.value.toUpperCase())} placeholder="Digite o código"/></div><small>O desconto é validado no servidor ao confirmar o pedido.</small></section>{error&&<div className="checkout-error">{error}</div>}</div><footer><div><span>Subtotal <b>{money.format(subtotal)}</b></span><span>Entrega <b>{money.format(fee)}</b></span><span className="total">Total antes do cupom <b>{money.format(subtotal+fee)}</b></span></div><button className="store-primary" disabled={sending||!cart.length} onClick={()=>void submit()}>{sending?"Enviando...":"Confirmar pedido"}</button></footer></section></div>;
}
