...
export interface Payment{id:string;companyId:string;orderId:string;orderNumber:number;customerName:string;amount:number;method:PaymentMethod;status:PaymentStatus;reference?:string;notes?:string;tenderedAmount?:number;changeAmount?:number;refundedAmount?:number;createdAt:string;updatedAt:string}
...
export interface PaymentRefund{id:string;companyId:string;paymentId:string;orderNumber:number;customerName:string;amount:number;reason:string;createdAt:string}
export interface RewardsState{version:1;companyId:string;coupons:Coupon[];payments:Payment[];program:LoyaltyProgram;accounts:LoyaltyAccount[];loyaltyTransactions?:LoyaltyTransaction[];paymentRefunds?:PaymentRefund[]}
